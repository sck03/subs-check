...
const routeHandlers = {
    ...
    async raw(request, url) {
        try {
            ...
            // 构建完整的 GitHub Raw URL
            const targetUrl = `https://raw.githubusercontent.com${rawPath}${url.search}`;
            ...
            return new Response(rawResponse.body, {
                ...
                headers: {
                    ...
                    'Cache-Control': 'max-age=3600' // 添加一小时缓存
                }
            });
        } catch (error) {
            return handleError('Raw 文件代理失败: ' + error.message);
        }
    }
};
